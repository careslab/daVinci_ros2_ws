...
class CameraRegistrationApplication:
    def __init__(self, ral, collection_mode, backup, psm_name, ecm_name, marker_size, expected_interval, camera, replay_file):
        ...
        if ecm_name is not None:
            ...
            self.ecm = None
        # event to wait for Enter GUI callback without busy-waiting
    ...
    def collect_poses_automatically(self):
        ...
        try:
            ...
            pass
        while self.ok and not self.enter_event.wait(timeout=self.expected_interval):
            print(self.ok, self.enter_event.is_set())
            ...
            # wait a short interval but allow early exit via enter_event
            if self.enter_event.wait(timeout=self.expected_interval):
                break

        # clear the event after it has been consumed so subsequent presses register
        try:
            self.enter_event.clear()
        except AttributeError:
            pass
    ...
    def determine_safe_range_of_motion(self):
        ...
        while True:
            ...
            # clear any pending enter event after returning from the collect loop
            try:
                self.enter_event.clear()
            except AttributeError:
                pass
    ...
    def _on_enter(self):
        ...
        try:
            ...
            # event may not be initialized in some contexts
            pass
    ...
    def run(self):
        ...
        try:
            ...
            if self.collection_mode != 'replay':
                ...
                self.enter = False
                # If running interactively in a terminal, block on input() until Enter.
                # Fall back to the event-based approach if input is not available.
                try:
                    input('Press Enter to continue...')
                    self.enter = True
                except (EOFError, KeyboardInterrupt):
                    try:
                        # fallback to event (e.g., when running with GUI callbacks)
                        self.enter_event.clear()
                        while self.ok and not self.enter_event.wait(timeout=self.expected_interval):
                            continue
                    except Exception:
                        # if all else fails, proceed
                        self.enter = True
